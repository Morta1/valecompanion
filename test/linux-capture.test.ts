...
describe("Linux capture mode configuration", () => {
  ...
  test("finds dumpcap from PATH when it is not in the default system directories", () => {
    ...
    const previousOverride = process.env.VALECOMPANION_DUMPCAP;
    const tempDir = mkdtempSync(join(tmpdir(), "valecompanion-dumpcap-"));
    ...
    try {
      ...
      delete process.env.VALECOMPANION_DUMPCAP;

      expect(findDumpcap()).toBe(dumpcapPath);
    } finally {
      process.env.PATH = previousPath;
      if (previousOverride === undefined) delete process.env.VALECOMPANION_DUMPCAP;
      else process.env.VALECOMPANION_DUMPCAP = previousOverride;
  ...
  test("finds Homebrew dumpcap in common install directories", () => {
    ...
    const previousPath = process.env.PATH;
    const previousOverride = process.env.VALECOMPANION_DUMPCAP;
    const tempHome = mkdtempSync(join(tmpdir(), "valecompanion-home-"));
