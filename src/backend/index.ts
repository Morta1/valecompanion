...
const root = path.resolve(process.env.VALECOMPANION_APP_ROOT ?? path.resolve(import.meta.dir, "../.."));
const applicationVersion = process.env.VALECOMPANION_VERSION ?? (() => {
...
const portable = existsSync(path.join(root, ".valecompanion-portable"));
...
const dataDirectory = path.resolve(process.env.VALECOMPANION_DATA_DIR
  ?? (portable ? path.join(root, "data") : path.join(defaultDataRoot ?? root, "Vale Companion")));
const rendererDirectory = path.resolve(process.env.VALECOMPANION_RENDERER_DIR ?? path.join(root, "resources", "views", "main"));
const logsDirectory = path.join(dataDirectory, "logs");
const diagnostics = createDiagnosticLogger("collector", process.env.VALECOMPANION_LOG_FILE ?? path.join(logsDirectory, "collector.log"));
...
process.stderr.write(`[valecompanion] collector ready on 127.0.0.1:${listeningPort}\n`);
