...
export class LinuxPcapRuntime {
  ...
  async status(): Promise<CaptureBackendStatus> {
    ...
    try {
      ...
      if (this.captureMode === "dumpcap") {
        detail = dumpcap
          ? "dumpcap mode active"
          : "dumpcap was not found; install Wireshark or set VALECOMPANION_DUMPCAP";
        if (!dumpcap) {
  ...
  async open(
    ...
    if (effectiveMode === "dumpcap") {
      if (!dumpcap)
        throw new Error(
          "dumpcap capture mode is selected but dumpcap was not found; install Wireshark or set VALECOMPANION_DUMPCAP",
        );
  ...
  private load(): PcapLibrary {
    ...
    const requested = process.env.VALECOMPANION_PCAP_LIBRARY;
...
export function findDumpcap(): string | undefined {
  const requested = process.env.VALECOMPANION_DUMPCAP;
  ...
  for (const entry of [
    "/home/linuxbrew/.linuxbrew/bin", // localref-allow
    "/home/linuxbrew/.linuxbrew/sbin", // localref-allow
    "/opt/homebrew/bin",
    "/opt/homebrew/sbin",
    "/usr/local/bin",
    "/usr/local/sbin",
