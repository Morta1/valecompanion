...
async function startCollector(): Promise<number> {
  ...
  collector = spawn(paths.runtime, [paths.entrypoint], {
    ...
    env: {
      ...process.env,
      VALECOMPANION_APP_ROOT: paths.installRoot,
      VALECOMPANION_DATA_DIR: paths.data,
      VALECOMPANION_RENDERER_DIR: paths.renderer,
      VALECOMPANION_VERSION: app.getVersion(),
      VALECOMPANION_LOG_FILE: path.join(paths.data, "logs", "collector.log"),
    },
  ...
  return new Promise<number>((resolve, reject) => {
    ...
    collector!.once("exit", (code, signal) => {
      ...
      if (!collectorReady) reject(new Error(`Collector exited before startup (${signal ?? code ?? "unknown"}).`));
        ...
        dialog.showErrorBox("Vale Companion collector stopped", `The capture collector exited unexpectedly (${signal ?? code ?? "unknown"}).`);
        app.quit();
      }
    });
  });
...
function runtimePaths(): RuntimePaths {
  ...
  const portable = existsSync(path.join(executableDirectory, ".valecompanion-portable"));
  return {
